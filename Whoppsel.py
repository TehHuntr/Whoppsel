...
# ====================== MAIN FUNCTIONALITY ======================
def order_detector(image_data           : np.ndarray,
    ...
    if (order_pix52_loc > (nx - 1)) or (order_pix52_loc < 0):
        raise ValueError(f"y-axis index for reference order location {order_pix52_loc} out of bounds for image shape [{0}:{nx - 1}, {0}:{ny - 1}]")
...
def order_tracer(image_data             : np.ndarray,
    ...
    for center in order_centers:
        ...
        for x in x_range:
            ...
            try:    
                y_rel = gaussian_centroid(image_data[hor_slice, x])
            except:
                y_rel = np.array([0])
        ...
        for ref in range(refine_count):
            ...
            for xx in xx_range:
                ...
                try:    
                    y_rel = gaussian_centroid(image_data[hor_slice, xx])
                except: 
                    y_rel = np.array([0])
...
def order_extractor(image_data  : np.ndarray,
                    order_locs  : pd.DataFrame,
                    ext_x_min   : int = 500,
                    ext_x_max   : int = 1500):
    '''
    ...
    for i in range(len(order_locs.index)):
        ...
        # Pass to private utility function
        y_data = _spectra_mask(
            x_data,
            order_locs["order_centers"][i],
            order_locs["order_low_edge"][i],
            order_locs["order_high_edge"][i],
            order_locs["ref_trace_coeffs"][i]
            )
        
        try:
            spectre_datas = []
            for j in range(len(x_data)):
                # List comprehension takes the uhh, what does this do again lmao
...
def cross_correlate(
    ...
    # Narrow given dataset to specified order: This takes all of data relating to a single specified order, 
    # for example 52, and creates a new dataset composed of all the different datasets for order 52
    # at different analysis variable values (e.g. temp = [10.0, 12.0, 13.0]).
    narrowed_datasets = []
    for i in range(len(datasets)):
        # Copy dataframe so the initial is not changed
    ...
    for i in range(len(narrowed_datasets)):
        ...
        left, right = int(corr_lag_cen - gauss_fit_region), int(corr_lag_cen + gauss_fit_region)
